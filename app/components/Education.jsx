...
import { educationData } from '../data/experience.js'
    ...
    >
      <div className={`lg:w-1/2 ${isLeft ? 'lg:pr-12 xl:pr-16' : 'lg:pl-12 xl:pl-16'} w-full`}>
        ...
        >
          ...
          {item.details && (
            <p className="text-(--text-primary)/70 text-xs sm:text-sm leading-relaxed mt-3 sm:mt-4">
              {item.details}
            </p>
          )}
          ...
          {item.link && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 text-(--accent-color) hover:text-(--text-primary) text-xs sm:text-sm mt-3 sm:mt-4 transition-colors duration-200"
            >
              View details
