...
const ExperienceCard = ({ item, index }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="absolute -left-4 sm:-left-5 top-6 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-linear-to-br from-(--border-color) to-(--accent-color) flex items-center justify-center shadow-md"
        >
        ...
        <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-(--text-primary) mt-1 leading-tight">
          {item.role}
        </h3>

        <p className="text-(--text-primary)/80 text-xs sm:text-sm mt-2 leading-relaxed">
          {item.organization}
          {item.location ? ` · ${item.location}` : ''}
        </p>

        {item.focus && (
          <p className="text-(--text-primary)/70 text-xs sm:text-sm mt-2 leading-relaxed">
            {item.focus}
          </p>
        )}

        {item.details?.length > 0 && (
          <ul className="mt-3 sm:mt-4 space-y-2 text-(--text-primary)/70 text-xs sm:text-sm">
            {item.details.map((detail, detailIndex) => (
              <li key={`${item.role}-${detailIndex}`} className="flex gap-2">
                <span className="text-(--border-color) flex-shrink-0 mt-0.5">•</span>
